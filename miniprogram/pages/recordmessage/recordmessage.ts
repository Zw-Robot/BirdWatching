...
import {wx_get_record,wx_delete_bird_record} from '../../components/interface'
...
Page({
  ...
  playClick() {
    ...
    audio.src = this.data.fronturl+this.data.record.bird_info[0].sound;
    audio.autoplay = true;
    console.log('播放录音');
  },
  ...
  deleteBirdRecord:function(){
    ...
    var date={
      ...
      openid:recordapp.globalData.openid,
      token:recordapp.globalData.token
    }
    console.log(this.data.query);
    
    wx_delete_bird_record(date).then(res=>{
      console.log(res);
    })
